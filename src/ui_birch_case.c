...
static const struct MonChoiceData sStarterTemplate[9] = 
{
...
static EWRAM_DATA struct MonChoiceData sStarterChoices[9];
static EWRAM_DATA struct MonChoiceData sWorkingStarters[9];

static EWRAM_DATA struct MonChoiceData sWorkingParty[4];
static EWRAM_DATA u8 sPartyCount = 0;
static EWRAM_DATA u16 sSelections[MAX_STARTER_PARTY];
...
static void GenerateRandomStarters(MainCallback);
...
static void CreateHandSprite()
{
    ...
    for(i=0; i<9; i++)
    {
        if(sWorkingStarters[i].species == SPECIES_NONE) // Choose Non Empty Slot To Start In
            continue;
    ...
    SampleUi_DrawMonIcon(sWorkingStarters[sBirchCaseDataPtr->handPosition].species);
...
static void CreatePokeballSprites()
{
    ...
    {
        u16 x, y;
        if(sWorkingStarters[i].species == SPECIES_NONE)
            continue;
...
static void BirchCase_RemoveMon()
{
    //sPartyCount should be lowered to the desired removable mon before running RemoveMon
    u16 lastChoice;
    sWorkingParty[sPartyCount].species = SPECIES_NONE;
    lastChoice = sSelections[sPartyCount];
    sWorkingStarters[lastChoice] = sStarterChoices[lastChoice];

}
static void BirchCase_GiveMon() // Function that calls the GiveMon function pulled from Expansion by Lunos and Ghoulslash
{
    
    u8 *evs = (u8 *) sWorkingStarters[sBirchCaseDataPtr->handPosition].evs;
    u8 *ivs = (u8 *) sWorkingStarters[sBirchCaseDataPtr->handPosition].ivs;
    u16 *moves = (u16 *) sWorkingStarters[sBirchCaseDataPtr->handPosition].moves;
    ...
    BirchCase_GiveMonParameterized(sWorkingStarters[sBirchCaseDataPtr->handPosition].species, sWorkingStarters[sBirchCaseDataPtr->handPosition].level, \
                sWorkingStarters[sBirchCaseDataPtr->handPosition].item, sWorkingStarters[sBirchCaseDataPtr->handPosition].ball, \
                sWorkingStarters[sBirchCaseDataPtr->handPosition].nature, sWorkingStarters[sBirchCaseDataPtr->handPosition].abilityNum, \
                sWorkingStarters[sBirchCaseDataPtr->handPosition].gender, evs, ivs, moves, \
                sWorkingStarters[sBirchCaseDataPtr->handPosition].ggMaxFactor, sWorkingStarters[sBirchCaseDataPtr->handPosition].teraType,\
                sWorkingStarters[sBirchCaseDataPtr->handPosition].isShinyExpansion);

    sWorkingParty[sPartyCount] = sWorkingStarters[sBirchCaseDataPtr->handPosition];
    sWorkingStarters[sBirchCaseDataPtr->handPosition].species = SPECIES_NONE;
...
void Task_OpenBirchCase(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        GenerateRandomStarters(CB2_ReturnToFieldContinueScriptPlayMapMusic);
        DestroyTask(taskId);
    }
}

static void GenerateRandomStarters(MainCallback callback)
{
    u8 i, j;
    u16 species;
    bool8 duplicate;

    // 1. Copy the template into the working array
    memcpy(sStarterChoices,sStarterTemplate,sizeof(sStarterTemplate));
    // 2. Loop through the array to replace SPECIES_NONE
    for (i = 0; i < 9; i++)
    {
        do
        {
            ...
            // Check for duplicates in the slots we've already filled
            for (j = 0; j < i; j++)
            {
                if (sStarterChoices[j].species == species)
                {
                    duplicate = TRUE;
                    break;
                }
        ...
        // 3. Assign the randomized species to the temporary map
        sStarterChoices[i].species = species;
    }
    memcpy(sWorkingStarters, sStarterChoices, sizeof(sStarterChoices));
    BirchCase_Init(callback);
...
static void PrintTextToBottomBar(u8 textId)
{
    ...
    u16 species = sWorkingStarters[sBirchCaseDataPtr->handPosition].species;
    ...
    if(sWorkingStarters[sBirchCaseDataPtr->handPosition].species == SPECIES_NONE)
    {
        PutWindowTilemap(WINDOW_BOTTOM_BAR);
        CopyWindowToVram(WINDOW_BOTTOM_BAR, 3);
        return;
...
static void Task_BirchCaseConfirmSelection(u8 taskId)
{
    if(JOY_NEW(A_BUTTON))
    {
        ...
        // 1. Record which grid slot was picked
        sSelections[sPartyCount] = sBirchCaseDataPtr->handPosition;
        
        // 2. Actually add to party
        BirchCase_GiveMon(); 
        
        // 3. Increment count
        sPartyCount++;

        if (sPartyCount < MAX_STARTER_PARTY)
        {
            PrintTextToBottomBar(CHOOSE_MON); // Prompt for next
            gTasks[taskId].func = Task_BirchCaseMain;
        }
        else
        {
...
static void Task_BirchCaseMain(u8 taskId)
{
    ...
    if(JOY_NEW(A_BUTTON))
    {
        if(sWorkingStarters[sBirchCaseDataPtr->handPosition].species != SPECIES_NONE) // If spot empty don't go to next control flow state
        {
            PlaySE(SE_SELECT);
    ...
    if(JOY_NEW(B_BUTTON))
    {
        if (sPartyCount > 0)
        {
            ...
            sPartyCount--; //LOWER COUNT FIRST SO REMOVE MON IS ON LAST MON FOR REMOVAL
            BirchCase_RemoveMon();
        }
        return;
    }
